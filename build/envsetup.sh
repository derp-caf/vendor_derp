# Derp-CAF functions that extend build/envsetup.sh
function __print_derp_functions_help() {
cat <<EOF
Additional Derp-CAF functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- derpemote:       Add a git remote for Derp-CAF github repository.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- mka:             Builds using SCHED_BATCH on all processors.
- mkap:            Builds the module(s) using mka and pushes them to the device.
- derpmka:         Cleans and builds using mka.
...
function derp_device_combos()
{
    local T list_file variant device

    T="$(gettop)"
    list_file="${T}/vendor/derp/derp.devices"
    variant="userdebug"
    ...
    do
        add_lunch_combo "derp_${device}-${variant}"
    done < "${list_file}"
}

function derp_rename_function()
{
    eval "original_derp_$(declare -f ${1})"
}

function _derp_build_hmm() #hidden
{
    printf "%-8s %s" "${1}:" "${2}"
}

function derp_append_hmm()
{
    HMM_DESCRIPTIVE=("${HMM_DESCRIPTIVE[@]}" "$(_derp_build_hmm "$1" "$2")")
}

function derp_add_hmm_entry()
{
    for c in ${!HMM_DESCRIPTIVE[*]}
    do
        if [[ "${1}" == $(echo "${HMM_DESCRIPTIVE[$c]}" | cut -f1 -d":") ]]
        then
            HMM_DESCRIPTIVE[${c}]="$(_derp_build_hmm "$1" "$2")"
            return
        fi
    done
    derp_append_hmm "$1" "$2"
...
function derpemote()
{
    ...
    git remote add derp "https://github.com/Derp-CAF/$project"
    echo "Remote 'derp' created"
}
...
function derp_push()
{
    ...
    git $path_opt push "https://github.com/Derp-CAF/$proj" "HEAD:$branch"
}


derp_rename_function hmm
function hmm() #hidden
{
    local i T
    T="$(gettop)"
    ...
    echo "vendor/derp extended functions. The complete list is:"
    for i in $(grep -P '^function .*$' "$T/vendor/derp/build/envsetup.sh" | grep -v "#hidden" | sed 's/function \([a-z_]*\).*/\1/' | sort | uniq); do
        echo "$i"
    done |column
}
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Derp-CAF model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var DERP_VERSION)
        ZIPFILE=Derp-CAF-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
...
export SDCLANG_AE_CONFIG=vendor/derp/build/sdclang/sdclangAE.json
export SDCLANG_CONFIG=vendor/derp/build/sdclang/sdclang.json
